			...
			if (dataObj.getXId() == dataObj2.getXId()) {    
				...
				if (dataObj.getLockType() == DataObj.READ) {    
					...
					if (dataObj2.getLockType() == DataObj.READ) {
						bitset.set(0, true);
						System.out.println("Want WRITE, lock conversion from shared to exclusive");
				...
				if (dataObj.getLockType() == DataObj.READ) {
					if (dataObj2.getLockType() == DataObj.WRITE) {
						...
						System.out.println("Want READ, someone has WRITE " + dataObj.getXId() + " " + dataObj2.getXId());
						return true;
					}
					else {
						System.out.println("Want READ, someone has READ, no conflict :) ");
						// do nothing, read request when there is a read lock, no conflict.
					}
				} else if (dataObj.getLockType() == DataObj.WRITE) {
					// transaction is requesting a WRITE lock and some other transaction has either
					// a READ or a WRITE lock on it ==> conflict
					System.out.println("Want WRITE, someone has READ or WRITE" + dataObj.getXId() + " " + dataObj2.getXId());
					return true;
				}
			}
		}
